...
# AIX Training Project 期中報告

## 專案簡介

AIX 是一套**自創的語音音素語言**（36 個音素），本專案從合成語音生成 → 特徵提取 → 模型訓練，完整實作一條機器學習 pipeline。
...
內建 **36 個 AIX 音素**（含邏輯符號），每類 6 個，用不同數學模型模擬發音：
...
| **共鳴音** | MX, NX, LX, RX, WX, JX | 鼻/邊/顫/近音，各異波形（方波、正弦、脈衝、頻率漸變） |
| **AI 專屬音** | ΞX, ΛX, ΨX, ΦX, ΔX, ΩX | 各異數學波形（脈衝串、和聲滑音等） |
| **邏輯符號** | ⟨⟩, ⟦⟧, ⟪⟫, ⌈⌉, ⌊⌋, ⌊⌈⌉⌋ | 用於標記句子結構，無對應發音 |
...
### AIX Tokenizer（42 個 token：6 特殊 + 36 音素）
